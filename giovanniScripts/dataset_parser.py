...
def process_text(tweet, remove_URL_and_mention=True):
    '''
...
def parse_file(file_to_parse, aggregation=1, file_to_save=None, verbose=False, remove_URL_and_mention=True, aggregate=True):
    '''
    ...
    for idx, doc in enumerate(documents):
        processed_tweet = process_text(doc.text, remove_URL_and_mention)
...
def filter_tweets(author, verbose=False):
    ...
    if verbose:
        print("      - Initial tweets = " + str(n_init_tweets))

    # HERE : insert filter

    if verbose:
        print("      - Saved tweets = " + str(len(tweets)))

    return tweets


def parse_tweets_from_dir(input_dir, output_dir=None, label=True, aggregation=100, verbosity_level=1, remove_URL_and_mention=True):
    '''
    ...
    for f in xml_files:
        ...
        try:
            author = parse_file(
                ...
                remove_URL_and_mention=remove_URL_and_mention
            )
        except:
