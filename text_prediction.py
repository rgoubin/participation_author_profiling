...
def classify_authors(Authors, classifiers, outputDic, classification_type='loose', verbosity=1):
    ...
    for auth in Authors:
        ...
        outputDic[auth["id"]].append(
            "{0:.2f}".format(gdr_predictions[0]))  # female
        outputDic[auth["id"]].append(
            "{0:.2f}".format(gdr_predictions[1]))  # male
    return Authors
...
def predict(inputPath, classifierPath, outputPath=None, verbosity_level=1, languages=['en', 'es'], is_dataset_2019=False):
    '''
